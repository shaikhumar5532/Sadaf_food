...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      const res = await axios.post("https://sadaf-food.onrender.com/api/auth/login", formData);
      if (res.data.success) {
        localStorage.setItem("token", res.data.token);
        localStorage.setItem("user", JSON.stringify(res.data.user));
        
        // Direct route based on user roles
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col justify-center items-center p-4">
      ...
      <div className="bg-white p-8 sm:p-12 rounded-3xl border border-gray-100 shadow-xl max-w-md w-full space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-gray-400">Log in to manage settings, wishlists, or administrative systems.</p>
        </div>
